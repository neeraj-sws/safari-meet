                                    ...
                                    @else
                                        @foreach ($safariConversations as $conversation)
                                            @php
                                                ...
                                                $chatUser = ($conversation->creator_id === $user->id &&
                                                    $conversation->creator_type === get_class($user))
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex-grow-1">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <span class="fw-semibold {{ $isActive ? 'text-white' : ''}}" style="font-size: 14px;" >{{ $chatUser->name }}</span>
                                                            <small
                                                                class="{{ $isActive ? 'text-white' : ' text-muted'}}" style="font-size: 12px;" >{{ optional($lastMessage)->created_at?->diffForHumans() }}</small>
                                                        </div>
                                                        <small
                                                            class=" {{ $isActive ? 'text-white' : ' text-muted'}} d-block" style="font-size: 12px;" >{{ Str::limit(optional($lastMessage)->message, 30) }}</small>
                                                    </div>
                                                </div>
                                            </a>
                                        @endforeach
                                ...
                                @if ($conversationId)
                                    ...
                                    <div class="d-flex align-items-center border-bottom bg-light p-3">
                                        @php
                                            ...
                                            $chatPartner = $firstMessage
                                                ? ($firstMessage->sender_id === $user->id &&  $firstMessage->sender_type === get_class($user)
                                    ...
                                    <div class="px-3 py-3 bg-white" style="height: 330px; overflow-y: auto;">
                                        @foreach ($chatConversationMessage as $message)
                                            @php $isMine = ($message->sender_id === $user->id && $message->sender_type === get_class($user)); @endphp
                                            <div
