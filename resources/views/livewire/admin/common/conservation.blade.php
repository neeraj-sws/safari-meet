            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" wire:click="addModel" class="btn btn-sm btn-light text-dark me-1">
                        {{ $conservation ? 'Add Details' : '+ Add Conservation' }}
                    </button>
                </div>

                <div class="card-body">
                    @if ($conservation)
                        <h5 class="mb-3">{{ $conservation->name }}</h5>
                        @if ($conservation->details->count())
                            <ul class="list-group list-group-flush">
                                @foreach ($conservation->details as $detail)
                                    <li class="list-group-item d-flex justify-content-between align-items-start">
                                        ...
                                        <button type="button"
                                            wire:click="deleteConservationDetail({{ $detail->id }})"
                        ...
                        @else
                            <p class="text-muted">No conservation details found.</p>
                        @endif
                    @else
                        <p class="text-muted">No conservation information available. Click “+ Add Conservation” to
                            begin.</p>
                    @endif
                </div>
            </div>
