<div class="container" id="details">
    @include('livewire.components.breadcrumb', [
        'menu' => $pageTitle,
        'submenus' => [['Package ' => route('admin.package.package')], 'Details'],
        'backButton' => true,
        'backUrl' => route('admin.package.package'),
    ])
    <div class="card">
        <div class="card-body">
    ...
    <div class="tab-content" id="pills-tabContent">
        <div class="tab-pane fade active show" role="tabpanel">
            @if ($showNavTab->id == 1)
                <livewire:admin.package.details.banner-images :package="$package" :characterstic="$hasActiveData" :key="'bannerImages-' . $characterstic->id" />
            @elseif ($showNavTab->id == 2)
                <livewire:admin.package.details.itinerary :package="$package" :characterstic="$hasActiveData" :key="'itinerary-' . $characterstic->id" />
            ...
            @elseif ($showNavTab->id == 8)
                <livewire:admin.package.details.seo-component :package="$package" :characterstic="$hasActiveData"
                    :key="'ratings-' . $characterstic->id" />
            @elseif ($showNavTab->id == 9)
                <livewire:admin.package.details.discussion :package="$package" :characterstic="$hasActiveData" :key="'discussion-' . $characterstic->id" />
            @else
