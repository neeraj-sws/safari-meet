    ...
    <div class="card">
        <div class="card-body">
            <form wire:submit.prevent="store">
                <div class="row">
                    <div class="col-md-12">
                        @php $editorId = 'parkruledonts-' . $this->getId(); @endphp
                        <livewire:admin.common.ckeditor-component model="Park Rules Dont's" :value="$dontsRules"
                            editor-id="{{ $editorId }}" wire:model.defer="dontsRules" />
                        @error('dontsRules')
                        ...
                        <div class="border border-1 p-4 rounded">
                            <div class="mb-3">
                                <label class="form-label">
                                    Upload Dont's Image <span class="text-danger">*</span>
                                </label>

                                <input type="file" wire:model="donts_image"
                                    accept=".jpg,.jpeg,.png,.webp,.JPG,.JPEG,.PNG,.WEBP"
                                    class="form-control @error('donts_image') is-invalid @enderror">
                            ...
                            <div class="border bg-light text-center p-2 position-relative"
                                ...
                                @if ($donts_image)
                                    <div wire:loading.delay wire:target="donts_image"
                                ...
                                <div wire:loading.remove wire:target="donts_image" class="h-100 position-relative">
                                    @if ($donts_image)
                                        <img src="{{ $donts_image->temporaryUrl() }}"
                                            class="img-fluid h-100 object-fit-contain">
                                        <button type="button"
                                            class="btn btn-sm btn-danger position-absolute top-0 end-0 rounded-circle z-3"
                                            style="padding:0.2rem 0.4rem" wire:click="removeDontsImage">
                                            ×
                                        </button>
                                    @elseif ($isEditing && !empty($dontsPreviousImage))
                                        <img src="{{ asset($dontsPreviousImage) }}"
                                            class="img-fluid h-100 object-fit-contain">
                                    @else
