<div>
    <!-- Add Diet Button -->
    <div class="col-lg-12 my-3 text-end">
        @if ($showDietModal == false)
            <button type="button" class="btn btn-primary" wire:click="addDiet">Add Diet Details</button>
        @else
            <button type="button" class="btn btn-danger" wire:click="$set('showDietModal', false)">Hide Diet
                Details</button>
        @endif
    </div>

    @if ($showDietModal)
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                @if ($showDietModal)
                    <form wire:submit.prevent="saveDiet">
                        <div class="modal-body">
                            <!-- Heading Input -->
                            <div class="mb-3">
                                <label class="form-label">Heading <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('heading') is-invalid @enderror"
                                    oninput="filterAndFormatInputs(this,{allowNumbers:true,capitalizeWords:true,allowAlpha:true,allowedSpecialChars:`,()|–&''/-`})"
                                    wire:model="heading">
                                @error('heading')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>

                            <!-- Image Upload -->
                            <div class="row">
                                <div class="col-md-6">
                                    <label class="form-label">Upload Image <span class="text-danger">*</span></label>
                                    <input type="file" wire:model="dietImage"
                                        accept=".jpg,.jpeg,.png,.webp,.JPG,.JPEG,.PNG,.WEBP"
                                        class="form-control @error('dietImage') is-invalid @enderror">
                                    <small>Expected image (320x240 px).</small> <br>
                                    @error('dietImage')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    <label>Preview</label>
                                    <div class="border bg-light text-center p-2 position-relative"
                                        style="height: 143px; overflow: hidden;">
                                        @if ($dietImage)
                                            <div wire:loading.delay wire:target="dietImage"
                                                class="d-flex align-items-center justify-content-center h-100 position-absolute w-100 bg-white z-3"
                                                style="top: 0; left: 0;">
                                                <div class="spinner-border text-primary" role="status">
                                                    <span class="visually-hidden">Uploading...</span>
                                                </div>
                                            </div>
                                        @endif
                                        <div wire:loading.remove wire:target="dietImage"
                                            class="h-100 position-relative">
                                            @if ($dietImage)
                                                <img src="{{ $dietImage->temporaryUrl() }}"
                                                    class="img-fluid h-100 object-fit-contain">
                                                <button type="button"
                                                    class="btn btn-sm btn-danger position-absolute top-0 end-0 rounded-circle z-3"
                                                    style="padding:0.2rem 0.4rem"
                                                    wire:click="removeDietImage">×</button>
                                            @elseif ($isEditing && !empty($previousImage))
                                                <img src="{{ asset($previousImage) }}"
                                                    class="img-fluid h-100 object-fit-contain">
                                            @else
                                                <span
                                                    class="text-muted d-flex align-items-center justify-content-center h-100">
                                                    No Image Selected
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <hr>
                            @php $editorId = 'AddDiet-' . $this->getId(); @endphp
                            <livewire:admin.common.ckeditor-component wire:model.defer="description"
                                editor-id="{{ $editorId }}" model="Add Diet Details" :value="$description"
                                :key="$editorId" />
                            @error('description')
                                <small class="text-danger d-block">{{ $message }}</small>
                            @enderror

                            <div class="text-end">

                                <button type="submit" class="btn btn-success text-end" wire:loading.attr="disabled">
                                    <span wire:loading.remove wire:target="saveDiet">Save</span>
                                    <span wire:loading wire:target="saveDiet">
                                        <span class="spinner-border spinner-border-sm" role="status"
                                            aria-hidden="true"></span>
                                        Saving...
                                    </span>
                                </button>
                            </div>
                        </div>
                    </form>
                @endif
            </div>
        </div>
    @endif


    <!-- Diet List -->
    <div class="row mt-4">
        @forelse ($dietData as $diet)
            <div class="col-md-6 mb-3">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                        <strong>{{ $diet->title }}</strong>
                        <div>
                            <button type="button" wire:click="editDiet({{ $diet->id }})"
                                class="btn btn-sm btn-light text-dark me-1">Edit</button>
                            <button type="button" wire:click="deleteDiet({{ $diet->id }})"
                                class="btn btn-sm btn-danger">Delete</button>
                        </div>
                    </div>
                    <div class="card-body">
                        @if (!empty($diet->short_description))
                            <small>{!! $diet->short_description !!}</small>
                        @else
                            <p class="text-muted">No details added yet.</p>
                        @endif
                    </div>
                </div>
            </div>
        @empty
            <div class="col-12">
                <p class="text-muted text-center">No Diets Details added yet.</p>
            </div>
        @endforelse
    </div>
</div>
