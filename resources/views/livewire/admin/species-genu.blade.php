                    ...
                    <form wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}">
                        <div class="mb-3" x-data="{ name: @entangle('name') }">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                ...
                                placeholder="Enter Species Genu Here...">
                            @error('name')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control select2 @error('family') is-invalid @enderror" id="family"
                                wire:model="family" placeholder="Select Family">
                                <option value="">Select Family</option>
                                @foreach ($species_family as $categoyId => $familyValue)
                                    <option value="{{ $familyValue->id }}" @selected($familyValue->id == $family)>
                                        {{ $familyValue->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="col-md-7">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <div class="position-relative">
                        <select class="form-control  " wire:model.live="family_filter" placeholder="Select Family">
                            <option value="">Select Family</option>
                            @foreach ($species_family as $categoyId => $familyValue)
                                <option value="{{ $familyValue->id }}" @selected($familyValue->id == $family)>
                                    {{ $familyValue->name }}
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($items as $index => $item)
                                    <tr wire:key="{{ $item->id }}">
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $item->species_category?->name }}</td>
                                        <td>{{ $item->species_family?->name }}</td>
                                        <td>
