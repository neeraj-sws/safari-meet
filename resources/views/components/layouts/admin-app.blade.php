...
<body>
    ...
    {{-- <script src="https://cdn.tiny.cloud/1/703e60ik4bbf0tgpid8nx2ir9yzwu22hdo6ab11waghkcofx/tinymce/8/tinymce.min.js" referrerpolicy="origin" crossorigin="anonymous"></script> --}}
    ...
    <script>
        function filterAndFormatInput(inputElement, allowNumbers = false, allowedSpecialChars = '', capitalizeWords =
            true) {

            let allowed = 'a-zA-Z\\s';

            if (allowNumbers) allowed += '0-9';

            const escapedSpecials = allowedSpecialChars.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
            if (escapedSpecials) allowed += escapedSpecials;

            const regex = new RegExp(`[^${allowed}]`, 'g');

            let value = inputElement.value.replace(regex, '').toLowerCase();

            if (capitalizeWords) {
                value = value.replace(/\b[a-zA-Z]/g, c => c.toUpperCase());
            }
    ...
    <script>
        function filterAndFormatInputs(inputElement, config = {}) {
            ...
            let newCursor = start - (oldValue.slice(0, start).match(regex)?.length || 0);

            if (trimSpaces) {
                let before = value.length;
                value = value.replace(/\s+/g, ' ').trim();
                newCursor -= (before - value.length);
            }
            ...
            if (maxLength && typeof maxLength === 'number') {
                ...
                newCursor = Math.min(newCursor, value.length);
            }
